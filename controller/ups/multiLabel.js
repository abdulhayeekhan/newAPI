...
//                 },
//                 UserCreatedForm: {
//                   UserCreatedFormFileName: `${CustomerContext}.txt`,
...
const GenMultiShipLabel = async (token,shipData,invoiceData) => {
  const shipmentData = shipData;
    try {
      ...
      const detailsContent = await invoiceData?.Details.map(detail => {
        return `Description: ${detail.description}, HtsCode: ${detail.HtsCode}, Unit Price: ${detail.unitPrice}, Quantity: ${detail.Qty}, Total : ${detail.Qty*detail.unitPrice}`;
      }).join('\n'); // Join each detail with a newline

      const totalPrice = await invoiceData?.Details?.reduce(
        (sum, item) => sum + (item.unitPrice * item.Qty || 0),
        0
      );
      // Convert the invoiceInfoData to text format
      const textContent = `Invoice Number: ${shipData?.ShipmentRequest?.Request?.TransactionReference?.CustomerContext}
      Total: ${totalPrice}
      SENDER: ESCM GMBH
      SENDER ADDRESS: Koln, Germany 50829
      SENDER PHONE: 0049-15202446893
      SENDER COUNTRY: Germany
      CONSIGNEE : ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Name}
      CONSIGNEE ADDRESS: ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Address?.AddressLine[0]} ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Address?.City}, ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Address?.StateProvinceCode} ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Address?.PostalCode} ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Address?.CountryCode}
      CONSIGNEE PHONE: ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Phone?.Number}
      CONSIGNEE COUNTRY: ${shipData?.ShipmentRequest?.Shipment?.ShipTo?.Address?.CountryCode}
      DETAILS: ${detailsContent}
      `;
      ...
      const paperlessID = paperless_res?.data?.UploadResponse?.FormsHistoryDocumentID?.DocumentID;
      shipmentData.ShipmentRequest.Shipment.ShipmentServiceOptions.InternationalForms.UserCreatedForm.DocumentID = [paperlessID];
      
      console.log("paperlessID",paperlessID)
      const response = await axios.post(
        `${UPS_API_URL}/api/shipments/v2409/ship?${query}`,
        shipmentData
        ,
